...
object ParallelCountChange extends ParallelCountChangeInterface {
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (threshold(money, coins) || money <= 0 || coins.isEmpty)
      ...
      val operands = parallel(countChange(money - coins.head, coins),
        countChange(money, coins.tail))
      operands._1 + operands._2
    }
  }
  ...
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = {
     ...
     * remaining coins is less than or equal to the starting money multiplied with the initial number of coins divided by 2:
     */
    (money, coins) =>

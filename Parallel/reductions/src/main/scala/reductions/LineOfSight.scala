...
object LineOfSight extends LineOfSightInterface {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    /**
     * Traverse the array of height values from the beginning to the end, and
     * write the maximum angle seen so far into the output array.
     * Output(0) should be 0
     */
    input.zipWithIndex.foreach({
      case (_, 0) => output(0) = 0
      case (h, i) => output(i) = Math.max(output(i - 1), h / i)
    })
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
              threshold: Int): Tree = {
    if ((end - from) < threshold + 1)
      Leaf(from, end, upsweepSequential(input, from, end))
    else {
      val mid = from + (end - from) / 2
      //      println("FROM: ", from, "MID: ", mid, "END: ", end)
      val (tL, tR) = parallel(upsweep(input, from, mid, threshold), upsweep
      (input, mid, end, threshold))
      Node(tL, tR)
    }
  }

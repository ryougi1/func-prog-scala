...
class BloxorzSuite {
  ...
  @Test def `neighbors with history level 1`: Unit =
    new Level1 {
      val neighborsHistory = neighborsWithHistory(Block(Pos(1, 1), Pos(1, 1)),
        List(Left, Up)).take(2).toList
      val correct = List(
        (Block(Pos(1, 2), Pos(1, 3)), List(Right, Left, Up)),
        (Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up))
      )
      assertEquals(neighborsHistory, correct)
    }

  @Test def `new neighbors only level 1`: Unit =
    new Level1 {
      ...
      val correct = Set((Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up)))
        .to(LazyList)
      assertEquals(newNeighbors, correct)
    }

  @Test def `remove me`: Unit =
    new Level1 {
      val paths = pathsToGoal
      println(paths.take(5) mkString ("\n"))
    }

...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block,
    List[Move])] = {
    /**
     * Want legalNeighbors, List[(Block, Move)]
     * Instead of Move, place it first in the list of history: List[Move]
     * Instead of List[(Block, List[Move])] use LazyList.
     */
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    // Check if any of the blocks in neighbors exist in explored
    // If so, remove them
    // Add all new blocks to explored
  ...
  /**
   * Breadth-first search: When we find the neighbors of our current state,
   * we explore each of them for each step. The respective neighbors of these
   * states are then stored to be explored at a later time.
   */
    if (initial.isEmpty) List.empty.to(LazyList)
    else {
      val more = for {
        (block, moves) <- initial
        (nextBlock, moreMoves) <- newNeighborsOnly(neighborsWithHistory
        (block, moves), explored)
      } yield (nextBlock, moreMoves)
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from(List((startBlock, List.empty[Move])).to(LazyList), Set(startBlock))
  ...
  lazy val solution: List[Move] = {
    val goalPaths = pathsToGoal
    if (goalPaths.isEmpty) List.empty[Move]
    else goalPaths(0)._2.reverse
  }
